...
const writeUnivInfo = async (univName, univBranch) => {
  const query = 'INSERT INTO university(univName, univBranch) VALUES (?,?);'
  const queryData = [univName, univBranch];

  const results = {};
  results.result = true;
  results.error = [];
  results.univName = univName;
  results.univBranch = univBranch;
